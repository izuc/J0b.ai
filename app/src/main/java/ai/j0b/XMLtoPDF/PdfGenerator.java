...
public class PdfGenerator {
    ...
    public final static int a4HeightInPoints = 842; // A4 height in points
    public final static int a4WidthInPoints = 595; // A4 width in points
                ...
                if (context != null) {
                    ...
                    for (int i = 0; i < viewList.size(); i++) {
                        ...
                        if (pageSize == PageSize.A4) {
                            final float MARGIN = 10f; // 1 inch margin on all sides
                            // Define the target printable area within the A4 page
                            float printableWidth = PdfGenerator.a4WidthInPoints - 2 * MARGIN;
                            float printableHeight = PdfGenerator.a4HeightInPoints - 2 * MARGIN;

                            // Measure the view to its natural size
                            content.measure(View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED),
                                    View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED));
                            content.layout(0, 0, content.getMeasuredWidth(), content.getMeasuredHeight());

                            // Calculate scale to fit the content within the printable area
                            float scaleX = printableWidth / (float) content.getMeasuredWidth();
                            float scaleY = printableHeight / (float) content.getMeasuredHeight();
                            float scale = Math.min(scaleX, scaleY); // Ensure content fits within the printable area

                            // Calculate offsets to center the content in the printable area
                            //float dx = (printableWidth - content.getMeasuredWidth() * scale) / 2f + MARGIN;
                            //float dy = (printableHeight - content.getMeasuredHeight() * scale) / 2f + MARGIN;

                            float dx = (PdfGenerator.a4WidthInPoints - content.getMeasuredWidth() * scale) / 2f; // Center horizontally
                            float dy = MARGIN; // Start from the top margin, moving down

                            // Create the page with full A4 dimensions
                            PdfDocument.PageInfo pageInfo = new PdfDocument.PageInfo.Builder(PdfGenerator.a4WidthInPoints, PdfGenerator.a4HeightInPoints, i + 1).create();
                            PdfDocument.Page page = document.startPage(pageInfo);

                            Canvas canvas = page.getCanvas();
                            canvas.save();
                            canvas.translate(dx, dy); // Center the content within the printable area
                            canvas.scale(scale, scale);

                            content.draw(canvas); // Draw the scaled content
            ...
            if (hasAllPermission(context) || (xmlToPDFLifecycleObserver != null && android.os.Build.VERSION.SDK_INT > 32)) {
                ...
                if(android.os.Build.VERSION.SDK_INT > 32) {
                    postFailure("Your current sdk is greater then 32, so you need to set ''xmlToPDFLifecycleObserver'' ." +
                            "To see example please check this code - https://github.ai.j0b/Android-XML-to-PDF-Generator/blob/master/sample/src/main/java/com/emon/exampleXMLtoPDF/MainActivity.java" +
                            ", line-67.");
                }else {
